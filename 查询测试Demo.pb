...
;          Debug "Flags = 0x" + Hex(DimTemp(k)\Flags, #PB_Word) + " : 0x" + Hex(DimTemp(k)\NameAddr, #PB_Long)
         ...
         If FindText$ = Result$                       ;如果实体名称一致,则视为找到匹配目标
            ...
            For i = 0 To CountAttri-1
               ...
               AttriIdx.q = (Attri >> 40) & $FFFFFF           ;40-63位为属性索引号
               TextAddr.q = Attri & $00FFFFFFFFFF           ;00-39位为属性值地址
               
               FileSeek(_Main\TextsID, TextAddr) 
               Notes$ = ReadString(_Main\TextsID, #PB_UTF8)
               Debug "AttriIndex = " + Str(AttriIdx) + ": 0x" + Hex(TextAddr)
               ShowText$ + "【"+_DimAttri$(AttriIdx)+"】"+Notes$ + #CRLF$
               Debug "【"+_DimAttri$(AttriIdx)+"】"+Notes$
            Next 
   ...
   Next 
   If IsFind = #False
      SetGadgetText(#rtxResult, "找不到目标: " + FindText$)
   EndIf 
EndProcedure

;查询事件
Procedure Event_btnScreen()
   Text$ = GetGadgetText(#txtEntrys)
   Find_Target(Text$)
EndProcedure
...
hWindow = OpenWindow(#winScreen, 0, 0, 400, 400, "整理程序", WindowFlags)
TextGadget  (#lblEntrys, 020, 020, 040, 020, "词条: ")
StringGadget(#txtEntrys, 060, 016, 250, 022, "灵峰")
ButtonGadget(#btnScreen, 330, 013, 060, 030, "查询")
EditorGadget(#rtxResult, 010, 050, 380, 340)
BindGadgetEvent(#btnScreen, @Event_btnScreen())
